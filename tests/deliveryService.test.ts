...
describe("createDelivery", () => {
  it("should create a new delivery and update the package with the delivery ID", async () => {
    ...
    jest
      ...
      .mockResolvedValue(mockPackage as any);
...
describe("updateDelivery", () => {
  it("should update the delivery and return the updated data", async () => {
    ...
    jest
      .spyOn(DeliveryModel, "findOneAndUpdate")
...
describe("updateDeliveryLocation", () => {
  it("should update the delivery location and return the updated delivery", async () => {
    ...
    const updatedLocation = { lat: 6.5, lng: 3.45 };
    const updatedDelivery = {
      ...mockDelivery,
...
describe("deleteDelivery", () => {
  it("should delete the delivery and unset the activeDeliveryId in the package", async () => {
    ...
    jest.spyOn(PackageModel, "findOne").mockResolvedValue(mockPackage as any);
    jest
      .spyOn(DeliveryModel, "findOneAndDelete")
  ...
  it("should throw an error if the delivery does not exist", async () => {
    ...
    await expect(
      deliveryService.deleteDelivery("NON_EXISTENT_DEL")
    ).rejects.toThrow("Delivery not found");
  });
});

describe("updateDelivery with rate limiting", () => {
  let ioMock: Server;

  beforeEach(() => {
    ioMock = new Server();
    jest.spyOn(ioMock, "to").mockReturnValue({
      emit: jest.fn(),
    } as any);
  ...
  it("should allow the first few requests and then block subsequent ones", async () => {
    const mockDelivery = {
      deliveryId: "DEL00007",
      packageId: "PKG00007",
      status: "open",
    ...
    const rateLimitedUpdateDelivery = async () => {
      try {
        await deliveryService.updateDelivery("DEL00007", "picked-up");
      } catch (error) {
    ...
    for (let i = 0; i < 5; i++) {
      const result = await rateLimitedUpdateDelivery();
      expect(result).toBe("success");
    }
    const result = await rateLimitedUpdateDelivery();
    expect(result).toBe("Too many requests. Please try again later.");

    expect(DeliveryModel.findOneAndUpdate).toHaveBeenCalledTimes(5);
  });
